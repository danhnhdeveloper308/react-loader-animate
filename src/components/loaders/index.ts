// Barrel export for all loaders — npm-ready
export { SpinLoader } from './SpinLoader';
export { DotsLoader } from './DotsLoader';
export { PulseLoader } from './PulseLoader';
export { WaveLoader } from './WaveLoader';
export { SquareLoader } from './SquareLoader';
export { FlipLoader } from './FlipLoader';
export { GradientSpinner } from './GradientSpinner';
export { OrbitLoader } from './OrbitLoader';
export { TriangleLoader } from './TriangleLoader';
export { DiamondLoader } from './DiamondLoader';
export { CrossLoader } from './CrossLoader';
export { ButterflyLoader } from './ButterflyLoader';
export { HexagonLoader } from './HexagonLoader';
export { SegmentLoader } from './SegmentLoader';
export { ArrowLoader } from './ArrowLoader';
export { GridLoader } from './GridLoader';
export { StarLoader } from './StarLoader';
export { PentagonLoader } from './PentagonLoader';
export { ChevronLoader } from './ChevronLoader';
export { SpiralLoader } from './SpiralLoader';
export { CornerSquaresLoader } from './CornerSquaresLoader';
export { SquareSplitLoader } from './SquareSplitLoader';
export { TriangleSplitLoader } from './TriangleSplitLoader';
export { CircleSplitLoader } from './CircleSplitLoader';
export { DiamondSplitLoader } from './DiamondSplitLoader';
export { HexagonSplitLoader } from './HexagonSplitLoader';
export { RingLoader } from './RingLoader';
export { ClockLoader } from './ClockLoader';
export { BarLoader } from './BarLoader';
export { BounceBallLoader } from './BounceBallLoader';
export { DNALoader } from './DNALoader';
export { HeartbeatLoader } from './HeartbeatLoader';
export { CubeLoader } from './CubeLoader';
export { InfinityLoader } from './InfinityLoader';
export { GearLoader } from './GearLoader';
export { PyramidLoader } from './PyramidLoader';
export { HourglassLoader } from './HourglassLoader';
export { RadarLoader } from './RadarLoader';
export { TypingDotsLoader } from './TypingDotsLoader';
export { PendulumLoader } from './PendulumLoader';
export { AtomLoader } from './AtomLoader';

// ── New loaders v2 ────────────────────────────────────────────────────────────
export { ColorRingLoader } from './ColorRingLoader';
export { CircularProgressLoader } from './CircularProgressLoader';
export { TailSpinLoader } from './TailSpinLoader';
export { BallTriangleLoader } from './BallTriangleLoader';
export { HashLoader } from './HashLoader';
export { SyncLoader } from './SyncLoader';
export { MutatingDotsLoader } from './MutatingDotsLoader';
export { ThreeDotsFadeLoader } from './ThreeDotsFadeLoader';

// ── New loaders v3 ────────────────────────────────────────────────────────────
export { Grid3x3Loader } from './Grid3x3Loader';
export { BarsLoader } from './BarsLoader';
export { RotatingLoader } from './RotatingLoader';

// ── New loaders v4 ── Ring/Circle ─────────────────────────────────────────────
export { RippleLoader } from './RippleLoader';
export { DoubleRingLoader } from './DoubleRingLoader';
export { ConcentricLoader } from './ConcentricLoader';
export { DashLoader } from './DashLoader';
export { CircleChaseLoader } from './CircleChaseLoader';
export { TrailSpinLoader } from './TrailSpinLoader';

// ── New loaders v4 ── Dots/Particles ──────────────────────────────────────────
export { BouncingDotsLoader } from './BouncingDotsLoader';
export { FlipDotsLoader } from './FlipDotsLoader';
export { CircleDotsLoader } from './CircleDotsLoader';
export { ChasingDotsLoader } from './ChasingDotsLoader';
export { GlowDotsLoader } from './GlowDotsLoader';
export { ParticleLoader } from './ParticleLoader';

// ── New loaders v4 ── Bars/Lines ──────────────────────────────────────────────
export { SoundBarsLoader } from './SoundBarsLoader';
export { HorizontalBarsLoader } from './HorizontalBarsLoader';
export { LineLoader } from './LineLoader';
export { ZigzagBallLoader } from './ZigzagBallLoader';
export { ProgressBarLoader } from './ProgressBarLoader';

// ── New loaders v4 ── Geometric ───────────────────────────────────────────────
export { CubeGridLoader } from './CubeGridLoader';
export { PacmanLoader } from './PacmanLoader';
export { TargetLoader } from './TargetLoader';
export { WiFiLoader } from './WiFiLoader';
export { MorphLoader } from './MorphLoader';
export { SquaresTrailLoader } from './SquaresTrailLoader';

// ── New loaders v4 ── Creative ────────────────────────────────────────────────
export { HeartLoader } from './HeartLoader';
export { FingerprintLoader } from './FingerprintLoader';
export { GalaxyLoader } from './GalaxyLoader';
export { BlobLoader } from './BlobLoader';
export { TunnelLoader } from './TunnelLoader';
export { MatrixLoader } from './MatrixLoader';
export { WindmillLoader } from './WindmillLoader';

// ── New loaders v5 ── Circle ──────────────────────────────────────────────────
export { SolarSystemLoader } from './SolarSystemLoader';
export { CometLoader } from './CometLoader';
export { GlowRingLoader } from './GlowRingLoader';
export { ChaseRingLoader } from './ChaseRingLoader';
export { BreathingRingLoader } from './BreathingRingLoader';

// ── New loaders v5 ── Grid ────────────────────────────────────────────────────
export { GridWaveLoader } from './GridWaveLoader';
export { GridBounceLoader } from './GridBounceLoader';
export { GridRainLoader } from './GridRainLoader';
export { GridPulseLoader } from './GridPulseLoader';
export { GridSnakeLoader } from './GridSnakeLoader';

// ── New loaders v6 ── Spinners ────────────────────────────────────────────────
export { NeonSpinLoader } from './NeonSpinLoader';
export { ArcSpinLoader } from './ArcSpinLoader';
export { GyroscopeLoader } from './GyroscopeLoader';
export { ElasticRingLoader } from './ElasticRingLoader';
export { VortexLoader } from './VortexLoader';
export { HalfSpinLoader } from './HalfSpinLoader';
export { PulsatingSquareLoader } from './PulsatingSquareLoader';
export { SphereDotsLoader } from './SphereDotsLoader';
export { OctaLoader } from './OctaLoader';
export { CylinderLoader } from './CylinderLoader';

// ── New loaders v6 ── Dots/Bounce ─────────────────────────────────────────────
export { StepDotsLoader } from './StepDotsLoader';
export { NeonDotsLoader } from './NeonDotsLoader';
export { QuadDotsLoader } from './QuadDotsLoader';
export { OrbitalDotsLoader } from './OrbitalDotsLoader';
export { MirrorDotsLoader } from './MirrorDotsLoader';
export { BubbleLoader } from './BubbleLoader';
export { WaterDropLoader } from './WaterDropLoader';
export { SatelliteLoader } from './SatelliteLoader';
export { MoleculeLoader } from './MoleculeLoader';
export { OscillateLoader } from './OscillateLoader';

// ── New loaders v6 ── Bars/Lines ──────────────────────────────────────────────
export { PianoKeysLoader } from './PianoKeysLoader';
export { LevelMeterLoader } from './LevelMeterLoader';
export { CountdownBarLoader } from './CountdownBarLoader';
export { StaircaseLoader } from './StaircaseLoader';
export { FilmStripLoader } from './FilmStripLoader';
export { ScanLoader } from './ScanLoader';
export { BatteryLoader } from './BatteryLoader';
export { DialLoader } from './DialLoader';
export { CompassLoader } from './CompassLoader';
export { GlitchLoader } from './GlitchLoader';

// ── New loaders v6 ── Geometric ───────────────────────────────────────────────
export { HexGridLoader } from './HexGridLoader';
export { CrosshairLoader } from './CrosshairLoader';
export { TesseractLoader } from './TesseractLoader';
export { KaleidoLoader } from './KaleidoLoader';
export { IsometricLoader } from './IsometricLoader';
export { GearTrainLoader } from './GearTrainLoader';
export { CubeRotateLoader } from './CubeRotateLoader';
export { SparkleLoader } from './SparkleLoader';
export { EclipseLoader } from './EclipseLoader';
export { CrystalLoader } from './CrystalLoader';

// ── New loaders v6 ── Creative/Nature ─────────────────────────────────────────
export { RocketLoader } from './RocketLoader';
export { SnowflakeLoader } from './SnowflakeLoader';
export { FireLoader } from './FireLoader';
export { LeafLoader } from './LeafLoader';
export { CircuitLoader } from './CircuitLoader';
export { LiquidFillLoader } from './LiquidFillLoader';
export { PlasmaLoader } from './PlasmaLoader';
export { NeoTrailLoader } from './NeoTrailLoader';
export { ThunderLoader } from './ThunderLoader';
export { BarrelLoader } from './BarrelLoader';

// ── New loaders v7 ── 20 unique additions ─────────────────────────────────────
export { HelixLoader } from './HelixLoader';
export { RippleSquareLoader } from './RippleSquareLoader';
export { TypewriterLoader } from './TypewriterLoader';
export { SonarLoader } from './SonarLoader';
export { NewtonLoader } from './NewtonLoader';
export { SignalLoader } from './SignalLoader';
export { CounterLoader } from './CounterLoader';
export { CubeUnfoldLoader } from './CubeUnfoldLoader';
export { SineWaveLoader } from './SineWaveLoader';
export { ChaseLoader } from './ChaseLoader';
export { SandTimer } from './SandTimer';
export { ShimmerLoader } from './ShimmerLoader';
export { GradientArcLoader } from './GradientArcLoader';
export { TriangleRingLoader } from './TriangleRingLoader';
export { DoubleHelixLoader } from './DoubleHelixLoader';
export { PulseRingLoader } from './PulseRingLoader';
export { BouncingLineLoader } from './BouncingLineLoader';
export { OrbLoader } from './OrbLoader';
export { FoldingLoader } from './FoldingLoader';
export { ConstellationLoader } from './ConstellationLoader';

export type { LoaderProps } from './types';
export { LOADER_SIZES, LOADER_BG_VARIANTS, LOADER_BORDER_VARIANTS, resolveColor, resolveSizeClass } from './types';
