...
export const GearLoader = /* @__PURE__ */ memo(({ size = 'md', variant = 'primary', color, visible = true, ariaLabel = 'loading', wrapperStyle, wrapperClass = '', animationDuration = 1.8 }: LoaderProps) => {
  ...
  return (
    <div role="status" aria-label={ariaLabel} className={wrapperClass} style={{ display: 'inline-flex', ...wrapperStyle, ...resolveColorStyle(variant, color) }}>
      <svg className={`${SIZE_MAP[size]} animate-spin-slow`} viewBox="0 0 24 24" style={{ animationDuration: `${animationDuration}s`, transformOrigin: 'center' }}>
        <path
